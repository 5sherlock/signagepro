...
export default function GroupManager({ devices, fetchDevices, stores, fetchStores, groups, fetchGroups, selectedStoreId, setSelectedStoreId }) {
  ...
  return (
    <div className="group-manager-container" style={{ display: 'flex', gap: '20px', height: 'calc(100vh - 120px)' }}>
      
      {/* 1. 왼쪽 사이드바: 사업장 목록 */}
      <div className="glass-card" style={{ width: '320px', display: 'flex', flexDirection: 'column', padding: '20px' }}>
      ...
      <div className="glass-card" style={{ flex: 1, overflowY: 'auto', padding: '24px', opacity: selectedStoreId ? 1 : 0.5, pointerEvents: selectedStoreId ? 'auto' : 'none' }}>
        {selectedStoreId ? (
          <>
