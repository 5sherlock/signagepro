...
const MediaThumb = ({ path, style = {} }) => {
  ...
  if (isVideo) return <video src={url} muted loop autoPlay playsInline style={{ objectFit: 'cover', ...style }} />;
...
const TransitionPreviewModal = ({ currentItem, nextItem, onChange, onClose }) => {
  ...
  const renderMedia = (item) => {
    ...
    return isVideo ? <video src={url} autoPlay muted style={{ width: '100%', height: '100%', objectFit: 'contain' }} /> : <img src={url} alt="" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />;
  };
  ...
  const getStyles = () => {
    ...
    if (transType === 'fade') {
      // 페이드: 사라졌다 나타남 (Fade to Black)
      // 0~0.5: layer1 사라짐, 0.5~1: layer2 나타남
    ...
    if (transType === 'slide') {
      return {
        layer1: { transform: `translateX(${-progress * 100}%)`, zIndex: 2 },
        layer2: { transform: `translateX(${(1 - progress) * 100}%)`, zIndex: 1 }
      };
    }
          ...
          <div className="timeline-container">
            ...
            <div className="track-lane">
              <div 
                ...
                style={{ 
                  width: `${(PRE_WAIT + transTime) / TOTAL_TIME * 100}%`,
                  background: `linear-gradient(90deg, var(--primary-blue), var(--primary-blue) ${PRE_WAIT / (PRE_WAIT + transTime) * 100}%, transparent)`
                }}
              ></div>
            </div>
            ...
            <div className="track-lane">
              <div 
                ...
                style={{ 
                  ...
                  background: `linear-gradient(90deg, transparent, var(--primary-orange) ${transTime / (transTime + 1000) * 100}%, var(--primary-orange))`
                }}
              ></div>
            </div>
...
const MediaManager = ({ stores = [], groups = [], devices = [], selectedStoreId, setSelectedStoreId, fetchDevices }) => {
  ...
  const handleDrop = (deviceId, media) => {
    const newItem = { mediaId: media.id, media, duration: 10, transition: 'fade', transitionTime: 1000, _key: `${media.id}-${Date.now()}` };
    setLanes(prev => {
      const updated = { ...prev };
