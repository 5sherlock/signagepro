...
const VideoWallManager = ({ stores = [], selectedStoreId, setSelectedStoreId }) => {
  ...
  // 미디어 스케줄링과 같은 키('SIGNAGE_UNDO_ACTIVE_GROUPS', 그룹 단위)를 공유 →
  // 어느 메뉴에서 배포하든 그룹 표식이 생겨 양쪽 메뉴에 동일하게 버튼이 뜬다.
  // 안 누르면 탭 세션 동안 유지, 새 탭/세션엔 숨김.
  ...
  const markGroupsActive = (gids) => setActiveGroups((prev) => { const n = new Set(prev); gids.forEach((g) => n.add(g)); return n; });
  ...
  const deploySet = async (g) => {
    ...
    try {
      ...
      markGroupsActive([...groups]); // 그룹 표식 → 미디어 스케줄링·동영상 양쪽에서 되돌리기 버튼 노출
            ...
            {sets.map((g) => (
              <div key={g.base}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                  ...
                  {(() => {
                    const gids = setGroupIds(g).filter((id) => activeGroups.has(id) && groupUndo[id]);
