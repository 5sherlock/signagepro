...
function SettingsTab({ onUnauth, deviceOrder = {} }) {
  ...
  const handleApkUpload = (e) => {
    ...
    // 파일명에서 버전 추출 (예: signagepro-0.4.5.apk → 0.4.5)
    const verMatch = file.name.match(/(\d+\.\d+\.\d+(?:\.\d+)?)/);
    if (verMatch) {
      localStorage.setItem('signagepro_apk_version', verMatch[1]);
      setLocalApkVersion(verMatch[1]);
    }
          ...
          {otaStatus === null ? (
            ...
            <>
              <span style={{ color: '#10B981', fontSize: '0.85rem' }}>
                ...
                {localApkVersion && (
                  <>&nbsp;<span style={{ background: 'rgba(16,185,129,0.15)', color: '#10b981', fontWeight: 700, padding: '1px 6px', borderRadius: '4px', fontSize: '0.8rem' }}>v{localApkVersion}</span></>
                )}
